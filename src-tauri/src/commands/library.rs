...
// canvas) and sends the JPEG bytes here to write into the app cache dir. Same
// path on macOS and iOS. Thumbnails live in Caches (regenerable — safe to lose).

use tauri::Manager;

/// Write `bytes` (a JPEG produced in the webview) to `<cache>/thumbs/{id}.jpg`
