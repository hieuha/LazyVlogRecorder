...
// webview picks a very high rate and the big per-chunk temp writes hitch capture.
// 12 Mbps is visually lossless at 1080p.
...
export function useRecorder(refs: UseRecorderRefs) {
  ...
  async function start(): Promise<void> {
    ...
    // Media Engine, freeing the CPU so the compositor can hit its frame rate. Falls
    // back to VP8 when H.264 isn't available or the encoder is forced to software.
    ...
    recRef.current = createRecorder({
      ...
      timesliceMs: 250,
      // Cap the hardware H.264 bitrate so chunks stay small (the uncapped default
      // was the cause of the earlier regression). VP8 keeps its default.
